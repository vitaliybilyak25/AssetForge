...
description: Product discovery and requirements analysis specialist for creating product briefs, requirements, market research, and problem-solution fit analysis. Use when creating product briefs, gathering requirements, or evaluating feature opportunities.
...
Forked Context Instructions
When invoked with `context: fork`, treat `$ARGUMENTS` as the feature, product area, or problem statement to analyze. If `$ARGUMENTS` is empty or ambiguous, ask concise clarification questions before producing a brief. Do not assume access to prior chat context.
...
See [REFERENCE.md](references/REFERENCE.md) for detailed interview frameworks and techniques.

Available Resources
•	references/REFERENCE.md - Detailed interview frameworks and techniques
...
Subagent Strategy
For broad product discovery, use parallel research agents only when the requested analysis is large enough to justify fan-out. Keep narrow feature briefs in the current forked context.
...
- Use current market data when web research tools are available
...
Notes for LLMs
•	Track multi-step workflow progress when the environment provides a task-tracking tool
...
•	Use web research for market research when the environment provides web tools
